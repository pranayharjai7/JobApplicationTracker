...
class GroqAIProvider : AIProvider {
    ...
    private val modelName = "llama-3.3-70b-versatile"
    private val gson = Gson()

    override suspend fun generateContent(prompt: String): String {
        if (BuildConfig.GROQ_API_KEY.isBlank()) {
            throw IllegalStateException("Groq API key is blank")
        }
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (responseCode in 200..299) {
                ...
                // Navigate format: {"choices": [{"message": {"content": "..."}}]}
                ...
                content ?: throw IllegalStateException("Groq returned empty content")
            } else {
                val errorStream = connection.errorStream?.let { InputStreamReader(it).readText() } ?: "No error body"
                throw IllegalStateException("Groq APIs failed with code: $responseCode Body: $errorStream")
            }
        }
    }
}
